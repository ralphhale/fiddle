...
{
    public class UpdateMaterialValidator : AbstractValidator<Material>
    {
        public UpdateMaterialValidator(IndustrialLightingContext dbContext)
        {
            ...
            RuleFor(item => item.Name)
                .MustAsync(async (item, name, cancellationToken) =>
                {
                    ...
                    var exists = await dbContext.Materials
                        .AnyAsync(item => item.Id != materialId && item.Name.ToLower() == name.ToLower());
